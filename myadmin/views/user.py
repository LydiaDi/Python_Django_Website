...
from myadmin.models import bUser
...
def index(request,pIndex=1):
    ...
    ulist = umod  #不进行过滤，包括已删除的信息所有的信息都给显示出来
    mywhere=[]
    #获取并判断搜索条件
    kw = request.GET.get("keyword",None)
    if kw:
        ulist = ulist.filter(Q(username__contains=kw) | Q(id__contains=kw))
